...
def test_admin_bootstrap_rotates_changed_password(
    ...
    monkeypatch.setattr(app_settings, "admin_email", "root@example.test")
    monkeypatch.setattr(app_settings, "admin_password", "first-admin-password-1")
    auth_module.bootstrap_admin()
    monkeypatch.setattr(app_settings, "admin_password", "second-admin-password-2")
    auth_module.bootstrap_admin()  # existing admin + changed password -> rotation
    ok = client.post(
        "/v1/auth/login",
        json={"email": "root@example.test", "password": "second-admin-password-2", "role": "admin"},
    )
    assert ok.status_code == 200
    stale = client.post(
        "/v1/auth/login",
        json={"email": "root@example.test", "password": "first-admin-password-1", "role": "admin"},
    )
...
def test_verify_token_cannot_reset_password(
    client: TestClient, mail_outbox: RecordingEmailSender
) -> None:
    _signup(client, "buyer")  # sends a VERIFY token
    verify_token = _extract_token(mail_outbox)
    r = client.post(
        "/v1/auth/password-reset/confirm",
        json={"token": verify_token, "new_password": "brand-new-password-1"},
    )
    assert r.status_code == 400


def test_reset_token_cannot_verify_email(
    client: TestClient, mail_outbox: RecordingEmailSender
) -> None:
    _signup(client, "buyer")
    mail_outbox.sent.clear()  # drop the verify email
    client.post(
        "/v1/auth/password-reset/request", json={"email": "kim@example.test", "role": "buyer"}
    )
    reset_token = _extract_token(mail_outbox)
    assert client.post("/v1/auth/verify", json={"token": reset_token}).status_code == 400
