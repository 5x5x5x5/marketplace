...
## Final-review amendments (2026-07-14)

A whole-branch final review found four findings (two critical, two important) plus two folded minors, fixed in one wave on top of this plan's Task 6 commit — see `.superpowers/sdd/final-review-fixes.md` for the full per-finding writeup and `docs/superpowers/specs/2026-07-14-disputes-design.md` (updated in the same wave) for the amended rules:

- **C1:** `transfer.reversed` is now downgraded to kind `ignored` unless the Stripe object's `reversed` field is `True` — a partial reversal (dispute clawback) no longer flips a paid payout to FAILED.
- **C2:** `resolve_dispute` now pins `refund_amount`/`clawback_amount` on the dispute (committed before either provider leg runs) once all 4xx guards pass; a retry with different amounts is rejected with 409 instead of silently diverging from a provider leg that may have already executed.
- **I1:** the clawback existence guard now also requires `Payout.status is PAID`, not just an existence + `provider_transfer_id` check — a FAILED payout has no money to claw back.
- **I2:** `resolve_dispute`'s status guard now rejects only `resolved` (not "not open") — a `chargeback_won`/`chargeback_lost` dispute can still be arbitrated, since one-dispute-per-job otherwise leaves no way to ever claw back an at-fault seller after a lost chargeback.
- Folded minors: the chargeback-lost webhook branch no longer books a $0.00 `chargeback_loss` row when `amount_minor` is absent/zero (the fee is still always booked); added tests for the previously-untested buyer/seller dispute GET reads.
- **Verification round:** the C2 pin's mid-function commit opened a lock-free window with two defects: (a) no status re-check after the re-lock, so a concurrent duplicate resolve double-booked the ledger; (b) the "re-lock" took the lock but read stale state — `get_one` with `with_for_update` does emit SELECT FOR UPDATE, yet with `expire_on_commit=False` hands back the unexpired identity-map object's old attributes (verified on Postgres with a SQL listener). Fixed with a `populate_existing` locked re-read + a 409 when the fresh status is already `resolved`, race-tested by simulating the winner inside the losing request's provider leg.

None of these changes touch the numbered task bodies above — they were applied as a fix wave, not a plan rewrite.
