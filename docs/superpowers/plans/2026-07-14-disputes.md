...
Verify: two tables; `sa.DateTime(timezone=True)` rendering; no `marketplace.` import; unique on `disputes.job_id`; indexes on `disputes.status`/`provider_dispute_id`/`buyer_id` and `adjustments.job_id`/`dispute_id`; six migrations apply from scratch.
